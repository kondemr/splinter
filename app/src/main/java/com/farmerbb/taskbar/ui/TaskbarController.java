...
public class TaskbarController extends UIController {
    ...
    private void initSeekBarPreference() {
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context);
        int seekBarValue = sharedPreferences.getInt(PREF_SEEKBAR, 0);
        adjustStartButtonPosition(seekBarValue);

        sharedPreferences.registerOnSharedPreferenceChangeListener(new SharedPreferences.OnSharedPreferenceChangeListener() {
            @Override
            public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
                if (PREF_SEEKBAR.equals(key)) {
                    int value = sharedPreferences.getInt(key, 0);
                    adjustStartButtonPosition(value);
                }
            }
        });
    }

    private void adjustStartButtonPosition(int marginStart) {
        if (startButton != null) {
